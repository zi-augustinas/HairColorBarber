...
namespace HairBarber
{
    public class ArrowButton : XRBaseInteractable
    {
        ...
        void Update()
        {
            if (m_LiftTheChair) ChangeChairsPosition(m_ChairMaximumPosition);

            if (m_LowerTheChair) ChangeChairsPosition(m_ChairOriginalPosition);
        }

        void ChangeChairsPosition(Vector3 destination)
        {
            if (Vector3.Distance(m_ChairTransform.position, destination) > 0.01f) m_ChairTransform.position = Vector3.Lerp(m_ChairTransform.position, destination, Time.deltaTime);
        }
